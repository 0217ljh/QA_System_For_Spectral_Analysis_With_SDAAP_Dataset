...
class Fine_tune():
    ...
    def load_tokenizer(self,
        ...
        Args:
            tokenizer_name (`str`, *optional*, defaults to `'meta-llama/Llama-2-7b-hf'`): 
        ...
        if self.tokenizer_name == 'chat-gpt': 
            ...
            try:
                self.tokenizer = AutoTokenizer.from_pretrained(self.tokenizer_name,token = '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',trust_remote_code=True)
                ...
                try:
                    self.tokenizer = LlamaTokenizer.from_pretrained(model_path,token = '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')
                except Exception as e:
                    print(e)
                    try:
                        self.tokenizer = AutoTokenizer.from_pretrained(model_path,token = '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')
                    except Exception as e:
                        raise
    ...
    def load_model(self, 
        ...
        Args:
            model_name (`str`, *optional*, defaults to `'meta-llama/Llama-2-7b-hf'`): 
                The character labels required for loading tokenizer through huggingface, which can be looked up on the huggingface and should also match the model you want to use.
